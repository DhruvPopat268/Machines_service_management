...
const getAll = async (req, res) => {
  try {
    ...
    if (stockStatus && ["In Stock", "Low Stock", "Out of Stock"].includes(stockStatus)) query.stockStatus = stockStatus;
...
const create = async (req, res) => {
  try {
    ...
    const machine = await Machine.create({
      ...
      currentStock:      0,
      stockStatus:       resolveStockStatus(0, threshold),
...
const update = async (req, res) => {
  try {
    ...
    if (lowStockThreshold !== undefined) {
      const threshold              = lowStockThreshold !== "" ? Number(lowStockThreshold) : -1;
      updateData.lowStockThreshold = threshold;
      updateData.stockStatus       = resolveStockStatus(machine.currentStock, threshold);
    }
...
const importMachines = async (req, res) => {
  try {
    ...
    const [allCategories, allDivisions] = await Promise.all([
    ...
    for (const [, entry] of machineMap) {
      ...
      try {
        ...
        const threshold = row.lowstockthreshold !== "" && row.lowstockthreshold !== undefined
