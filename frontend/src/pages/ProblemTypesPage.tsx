...
const emptyForm = { name: "", description: "", status: "Active" as ProblemType["status"] };
const LIMIT = 10;
...
const ProblemTypesPage = () => {
  ...
  const [editDialog, setEditDialog] = useState<ProblemType | null>(null);
  ...
  const fetchProblemTypes = useCallback(async (page = 1) => {
    ...
    try {
      ...
      const res = await api.get("/admin/problem-types", { params, signal: controller.signal });
      ...
      if (err?.name !== "CanceledError" && err?.code !== "ERR_CANCELED") {
        toast.error("Failed to fetch problem types");
      }
    } finally {
  ...
  useEffect(() => { fetchProblemTypes(1); }, [fetchProblemTypes]);

  const handleAdd = async () => {
    if (!addForm.name) return toast.error("Name is required");
    setSubmitting(true);
    try {
      await api.post("/admin/problem-types", addForm);
      toast.success("Problem type added successfully");
      setAddDialog(false);
      setAddForm(emptyForm);
      fetchProblemTypes(1);
    } catch (err: any) {
      toast.error(err.response?.data?.message || "Failed to add problem type");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleEdit = async () => {
    ...
    setSubmitting(true);
    try {
      await api.patch(`/admin/problem-types/${editDialog._id}`, editForm);
      toast.success("Problem type updated successfully");
      setEditDialog(null);
      fetchProblemTypes(pagination.page);
    } catch (err: any) {
      toast.error(err.response?.data?.message || "Failed to update problem type");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    setSubmitting(true);
    try {
      await api.delete(`/admin/problem-types/${deleteDialog._id}`);
      ...
      fetchProblemTypes(newPage);
    } catch (err: any) {
      toast.error(err.response?.data?.message || "Failed to delete problem type");
    } finally {
      setSubmitting(false);
    }
  };

  const toggleStatus = async (pt: ProblemType) => {
    const newStatus = pt.status === "Active" ? "Inactive" : "Active";
    try {
      await api.patch(`/admin/problem-types/${pt._id}`, { status: newStatus });
  ...
  const handleDownloadSample = async () => {
    try {
      const res = await api.get("/admin/problem-types/sample", { responseType: "blob" });
      const url = URL.createObjectURL(res.data);
      const a = document.createElement("a");
      a.href = url; a.download = "problem_types_sample.xlsx"; a.click();
  ...
  const handleImportUpload = async () => {
    ...
    try {
      ...
      const res = await api.post("/admin/problem-types/import", form, { headers: { "Content-Type": "multipart/form-data" } });
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (fromDate) params.fromDate = toISTDateParam(fromDate);
      if (toDate)   params.toDate   = toISTDateParam(toDate);
      const res = await api.get("/admin/problem-types/export", { params, responseType: "blob" });
      const url = URL.createObjectURL(res.data);
      const a = document.createElement("a");
      a.href = url; a.download = "problem_types.xlsx"; a.click();
      URL.revokeObjectURL(url);
    } catch {
  ...
  const columns: Column<ProblemType>[] = [
    { key: "_id", label: "No.", render: (_p, i) => <span className="font-medium text-foreground">{(pagination.page - 1) * LIMIT + i + 1}</span> },
    ...
    },
    {
      key: "actions", label: "Actions", render: (p) => (
        <div className="flex items-center gap-1">
          <Button variant="ghost" size="icon" className="h-8 w-8" aria-label={`Edit ${p.name}`} onClick={() => { setEditDialog(p); setEditForm({ name: p.name, description: p.description, status: p.status }); }}>
            <Edit className="h-4 w-4" />
          </Button>
          <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive hover:text-destructive" aria-label={`Delete ${p.name}`} onClick={() => setDeleteDialog(p)}>
  ...
  return (
    <div className="space-y-6">
      {loading ? <Spinner /> : (
        <>
          ...
          <Pagination
            ...
            pageSize={LIMIT}
            onPageChange={fetchProblemTypes}
          />
        </>
      )}
      ...
      <Dialog open={addDialog} onOpenChange={(open) => { if (!open) { setAddDialog(false); setAddForm(emptyForm); } }}>
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2"><Label htmlFor="add-pt-name">Name</Label><Input id="add-pt-name" placeholder="e.g. Mechanical Failure" value={addForm.name} onChange={(e) => setAddForm((p) => ({ ...p, name: e.target.value }))} /></div>
            <div className="space-y-2"><Label htmlFor="add-pt-desc">Description</Label><Textarea id="add-pt-desc" placeholder="Brief description of this problem type" value={addForm.description} onChange={(e) => setAddForm((p) => ({ ...p, description: e.target.value }))} /></div>
            <div className="space-y-2">
              <Label htmlFor="add-pt-status">Status</Label>
              <Select value={addForm.status} onValueChange={(v) => setAddForm((p) => ({ ...p, status: v as ProblemType["status"] }))}>
                <SelectTrigger id="add-pt-status"><SelectValue /></SelectTrigger>
                <SelectContent>
      ...
      <Dialog open={!!editDialog} onOpenChange={(open) => !open && setEditDialog(null)}>
        <DialogContent>
          <DialogHeader><DialogTitle>Edit Problem Type</DialogTitle></DialogHeader>
          <div className="space-y-4 py-4">
            <div className="space-y-2"><Label htmlFor="edit-pt-name">Name</Label><Input id="edit-pt-name" value={editForm.name} onChange={(e) => setEditForm((p) => ({ ...p, name: e.target.value }))} /></div>
            <div className="space-y-2"><Label htmlFor="edit-pt-desc">Description</Label><Textarea id="edit-pt-desc" value={editForm.description} onChange={(e) => setEditForm((p) => ({ ...p, description: e.target.value }))} /></div>
            <div className="space-y-2">
              <Label htmlFor="edit-pt-status">Status</Label>
              <Select value={editForm.status} onValueChange={(v) => setEditForm((p) => ({ ...p, status: v as ProblemType["status"] }))}>
                <SelectTrigger id="edit-pt-status"><SelectValue /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="Active">Active</SelectItem>
                  <SelectItem value="Inactive">Inactive</SelectItem>
                </SelectContent>
      ...
      <Dialog open={importDialog} onOpenChange={(open) => { if (!open) { setImportDialog(false); setImportStep("menu"); setImportFile(null); } }}>
        <DialogContent>
          {importStep === "menu" && (
            <>
              <DialogHeader><DialogTitle>Import Problem Types</DialogTitle><DialogDescription>Download the sample file, fill in your data, then upload.</DialogDescription></DialogHeader>
          ...
          {importStep === "confirm" && (
            <>
              <DialogHeader><DialogTitle>Upload Problem Types</DialogTitle><DialogDescription>Please confirm you have checked the sample file and your file matches the required format before uploading.</DialogDescription></DialogHeader>
          ...
          {importStep === "upload" && (
            <>
              <DialogHeader><DialogTitle>Select File</DialogTitle><DialogDescription>Select a .xlsx file to import problem types.</DialogDescription></DialogHeader>
              <div className="py-4">
      ...
      <Dialog open={exportDialog} onOpenChange={setExportDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Export Problem Types</DialogTitle>
            <DialogDescription>Do you want to download all problem types as an Excel file?</DialogDescription>
          </DialogHeader>
          <DialogFooter>
