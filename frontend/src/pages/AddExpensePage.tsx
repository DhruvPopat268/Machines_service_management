...
const AddExpensePage = () => {
  ...
  const [editDialog, setEditDialog]   = useState<Expense | null>(null);
  ...
  useEffect(() => {
    api.get("/admin/expenses", { params: { limit: "200", status: "Active" } })
  ...
  const fetchExpenses = useCallback(async (page = 1) => {
    ...
    try {
      ...
      const res = await api.get("/admin/add-expenses", { params, signal: controller.signal });
      setData(res.data.data.expenses);
      setTotalExpense(res.data.data.stats.totalExpense ?? 0);
      setPagination({
  ...
  const handleAdd = async () => {
    ...
    setSubmitting(true);
    try {
      await api.post("/admin/add-expenses", {
  ...
  const handleEdit = async () => {
    ...
    setSubmitting(true);
    try {
      await api.patch(`/admin/add-expenses/${editDialog._id}`, {
  ...
  const handleDelete = async () => {
    ...
    setSubmitting(true);
    try {
      await api.delete(`/admin/add-expenses/${deleteDialog._id}`);
  ...
  const handleDownloadSample = async () => {
    try {
      const res = await api.get("/admin/add-expenses/sample", { responseType: "blob" });
      const url = URL.createObjectURL(res.data);
      const a = document.createElement("a");
      a.href = url; a.download = "expenses_sample.xlsx"; a.click();
  ...
  const handleImportUpload = async () => {
    ...
    try {
      ...
      const res = await api.post("/admin/add-expenses/import", form, { headers: { "Content-Type": "multipart/form-data" } });
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (appliedFromDate) params.fromDate   = appliedFromDate;
      if (appliedToDate)   params.toDate     = appliedToDate;
      const res = await api.get("/admin/add-expenses/export", { params, responseType: "blob" });
      const url = URL.createObjectURL(res.data);
      const a = document.createElement("a");
  ...
  const handleQuickAddCategory = async (targetForm: "add" | "edit") => {
    ...
    try {
      const res = await api.post("/admin/expenses", { name: quickCatName.trim(), status: "Active" });
      const newCat: ExpenseCategory = res.data.data;
    ...
    <div className="space-y-6">
      {loading ? <Spinner /> : (
        <>
          ...
          {/* Records per page left, total expense right */}
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Label className="text-xs text-muted-foreground whitespace-nowrap">Records per page</Label>
              <Select value={String(limit)} onValueChange={(v) => { setLimit(Number(v)); fetchExpenses(1); }}>
        ...
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <Input placeholder="e.g. Travel" value={quickCatName} onChange={(e) => setQuickCatName(e.target.value)} onKeyDown={(e) => e.key === "Enter" && quickCatDialog && handleQuickAddCategory(quickCatDialog)} autoFocus />
            </div>
          </div>
          <DialogFooter>
      ...
      <Dialog open={!!deleteDialog} onOpenChange={(open) => { if (!open) setDeleteDialog(null); }}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to delete this expense of <span className="font-semibold text-foreground">₹{deleteDialog?.amount.toLocaleString("en-IN")}</span>? This action cannot be undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
      ...
      <Dialog open={importDialog} onOpenChange={(open) => { if (!open) { setImportDialog(false); setImportStep("menu"); setImportFile(null); } }}>
        <DialogContent>
          {importStep === "menu" && (
            <>
              <DialogHeader><DialogTitle>Import Expenses</DialogTitle><DialogDescription>Download the sample file, fill in your data, then upload.</DialogDescription></DialogHeader>
          ...
          {importStep === "confirm" && (
            <>
              <DialogHeader><DialogTitle>Upload Expenses</DialogTitle><DialogDescription>Please confirm you have checked the sample file and your file matches the required format before uploading.</DialogDescription></DialogHeader>
          ...
          {importStep === "upload" && (
            <>
              <DialogHeader><DialogTitle>Select File</DialogTitle><DialogDescription>Select a .xlsx file to import expenses.</DialogDescription></DialogHeader>
              <div className="py-4">
      ...
      <Dialog open={exportDialog} onOpenChange={setExportDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Do you want to download the current filtered expenses as an Excel file?</DialogDescription>
          </DialogHeader>
          <DialogFooter>
