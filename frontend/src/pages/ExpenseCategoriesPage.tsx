...
const emptyForm = { name: "", status: "Active" as ExpenseCategory["status"] };
const LIMIT = 10;
...
const ExpenseCategoriesPage = () => {
  ...
  const [editDialog, setEditDialog] = useState<ExpenseCategory | null>(null);
  const [editForm, setEditForm] = useState(emptyForm);

  const [deleteDialog, setDeleteDialog] = useState<ExpenseCategory | null>(null);
  ...
  const fetchCategories = useCallback(async (page = 1) => {
    ...
    try {
      ...
      const res = await api.get("/admin/expenses", { params, signal: controller.signal });
  ...
  const handleAdd = async () => {
    ...
    setSubmitting(true);
    try {
      await api.post("/admin/expenses", addForm);
  ...
  const handleEdit = async () => {
    ...
    setSubmitting(true);
    try {
      await api.patch(`/admin/expenses/${editDialog._id}`, editForm);
  ...
  const handleDelete = async () => {
    ...
    setSubmitting(true);
    try {
      await api.delete(`/admin/expenses/${deleteDialog._id}`);
  ...
  const toggleStatus = async (cat: ExpenseCategory) => {
    const newStatus = cat.status === "Active" ? "Inactive" : "Active";
    try {
      await api.patch(`/admin/expenses/${cat._id}`, { status: newStatus });
    ...
    <div className="space-y-6">
      {loading ? <Spinner /> : (
        <>
          <PageHeader
            ...
            description="Manage expense categories for classifying expenses"
        ...
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              <Label htmlFor="add-exp-name">Category Name</Label>
              <Input id="add-exp-name" placeholder="e.g. Travel" value={addForm.name} onChange={(e) => setAddForm((p) => ({ ...p, name: e.target.value }))} />
            </div>
            <div className="space-y-2">
              <Label htmlFor="add-exp-status">Status</Label>
              <Select value={addForm.status} onValueChange={(v) => setAddForm((p) => ({ ...p, status: v as ExpenseCategory["status"] }))}>
                <SelectTrigger id="add-exp-status"><SelectValue /></SelectTrigger>
                <SelectContent>
      ...
      <Dialog open={!!editDialog} onOpenChange={(open) => !open && setEditDialog(null)}>
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              <Label htmlFor="edit-exp-name">Category Name</Label>
              <Input id="edit-exp-name" value={editForm.name} onChange={(e) => setEditForm((p) => ({ ...p, name: e.target.value }))} />
            </div>
            <div className="space-y-2">
              <Label htmlFor="edit-exp-status">Status</Label>
              <Select value={editForm.status} onValueChange={(v) => setEditForm((p) => ({ ...p, status: v as ExpenseCategory["status"] }))}>
                <SelectTrigger id="edit-exp-status"><SelectValue /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="Active">Active</SelectItem>
                  <SelectItem value="Inactive">Inactive</SelectItem>
                </SelectContent>
